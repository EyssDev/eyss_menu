...
function SetDisplay(bool)
    ...
    ESX.TriggerServerCallback('eyss_menu:getPlayerName', function(playername)
        SendNUIMessage({action = 'playername', value = playername})
    end)
    
    -- Phone Number
    ESX.TriggerServerCallback('eyss_menu:getPhoneNumber', function(phonenumber)
    ...
    -- Money in hand (cash)
    ESX.TriggerServerCallback('eyss_menu:getCash', function(walletamount)
        SendNUIMessage({action = 'walletamount', value = walletamount})
    end)

    -- Bank Money
    ESX.TriggerServerCallback('eyss_menu:getBank', function(bankamount)
...
print("[^1Auteur^0] : ^5github.com/EyssDev^0")
