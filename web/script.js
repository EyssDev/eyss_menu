$(function () {
    ...
    document.onkeyup = function (data) {
        if (data.which == 27) {
            $("html").fadeOut("slow");
            $.post('https://eyss_menu/close', JSON.stringify({}));
            return
        }
    };

    $("#inventory").click(function () {
        $.post('https://eyss_menu/inventory', JSON.stringify({}));
        return
    })

    $("#billing").click(function () {
        $.post('https://eyss_menu/billing', JSON.stringify({}));
        return
    })

    $("#cardealership").click(function () {
        $.post('https://eyss_menu/cardealership', JSON.stringify({}));
        return
    })
    
    $("#shop").click(function () {
        $.post('https://eyss_menu/shop', JSON.stringify({}));
        return
    })
    
    $("#administratorcontact").click(function () {
        $.post('https://eyss_menu/administratorcontact', JSON.stringify({}));
        return
    })
    
    $("#jobcenter").click(function () {
        $.post('https://eyss_menu/jobcenter', JSON.stringify({}));
        return
    })
})